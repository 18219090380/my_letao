...
//功能需求7:由于退出了登录系统,但是,通过url栏直接输入首页url,也会自动跳转到首页,那么,我们可以控制一下登录之前不给其跳转
//7.1为了控制初始状态,没登录之前,直接让其在登录页面,不能进其他页面(可以通过开始发送ajax请求,如果是真,那么可以让其跳转)
//$.ajax({
...
//    }
//})
//7.2由于上面的一直发ajax请求,不可以;那么,可以判断只要不存在登录页面,都发送一次ajax请求
...
//功能需求7:导航栏点击每一个li标签,左边框都会有3px的边框线显示,并且,背景颜色也变
